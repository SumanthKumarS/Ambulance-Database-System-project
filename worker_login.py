...
class Login :
        ...
        # ------------if the email and password are not blank
            ...
            try :
                ...
                if row != None :
                    ...
                    # ---- title bar ------ #
                    title = Label(self.root2 , text='Worker Details Report' , bd=10 , relief=GROOVE ,
    ...
    def profile_page(self) :
        ...
        dob_txt = DateEntry(profile_frame , textvariable=self.dob_var , font=('times new roman' , 13 , 'bold') , bd=0 ,
                        state='readonly')
    ...
    def update_page(self) :
        ...
        dob.place(x=250 , y=180 , height=20 , width=70)
        dob_txt = DateEntry(update_frame , textvariable=self.dob_var , font=('times new roman' , 13 , 'bold') , bd=0)
        dob_txt.place(x=350 , y=180 , height=20 , width=130)
        ddate = self.dob_var.get()
        if (ddate) :
            ...
            mon = int(ddate[1])
            print(mon)
            self.dbdate = '20' + ddate[2] + '-' + ddate[1] + '-' + ddate[0]
            print(self.dbdate)
    ...
    def update_data(self) :
        ...
        cur = con.cursor()
        cur2 = con.cursor()
        print(self.id_var.get(),self.name_var.get(),self.phone_var.get(),self.email_var.get(),self.dbdate,self.address_var.get())
        cur2.execute("select * from worker_manage where email=%s" , (self.txt_email.get()))
        row = cur2.fetchone()
        print(row[0])
